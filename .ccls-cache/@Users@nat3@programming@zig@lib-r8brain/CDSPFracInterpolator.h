...
public:
	...
	virtual int getInLenBeforeOutPos( const int ReqOutPos ) const
	{
		if( IsWhole )
		{
			return( Latency + (int) (( InitFracPosW + (double) ReqOutPos *
				InStep ) / OutStep ));
		}

		return( Latency + (int) ( InitFracPos + (double) ReqOutPos *
			SrcSampleRate / DstSampleRate ));
	}

	virtual const char* getName() const
	{
		return( IsWhole ? "CDSPFracInterpolator(Whole)" : "CDSPFracInterpolator(Fractional)" );
	}
